...
if [[ ! -f $filename ]]; then
    ...
    echo -n "Threadpool Size","Sleep Time (ms)","Concurrent Users", > $filename
...
do
    for sleep_time_dir in $(find $threadpool_size_dir -maxdepth 1 -name '*ms_sleep' | sort -V)
    do
        ...
        do
            ...
            echo -n "$threadpool_size,$sleep_time,$concurrent_users" >> $filename
